...
const displayCategories = ["All", "Website Developing", "Video Editing", "Graphic Designing"]
const categoryMap = {
  "All": "All",
  "Website Developing": "website-developing",
...
export default function Portfolio() {
  ...
  const filtered = useMemo(() => {
    ...
    if (active === "Website Developing") {
      filteredProjects = filteredProjects.filter((p) => p.liveLink)
    }
          ...
          <AnimatePresence mode="popLayout">
            {filtered.length > 0 ? (
              ...
              <motion.article
                ...
                exit={{ opacity: 0, y: -20 }}
                transition={{ duration: 0.4 }}
                className="relative overflow-hidden rounded-2xl border bg-white dark:bg-neutral-900 shadow-sm col-span-full text-center p-10"
              >
                <h3 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">
                  Coming Soon
                </h3>
